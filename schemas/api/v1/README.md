...
which is stdlib-only. `T-005` runs before `T-008`, so there is no `web/`, no
...
(ADR 0001 invariant 5), and handed `T-008` a scaffold it did not choose (D-029).
...
**Known gap (R17):** that check was run by hand. Nothing in CI compiles `types.d.ts`,
because there is no Node job until `T-008` — which must add `tsc --noEmit` when it brings
one in. The drift test proves the file matches the generator, not that it compiles.
