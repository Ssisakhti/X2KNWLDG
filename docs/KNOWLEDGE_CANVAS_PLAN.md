...
| D-210 | Persian/RTL text survives every qualified route intact; the routes differ only on **link representation**, so the canonical text is the **authored** form and cross-route comparison normalizes URLs first | accepted | ZWNJ, Persian ye, keheh and Persian digits identical across Tier 1 and FxTwitter on four Persian posts; ZWNJ present in 53 of 60 sampled posts. Three of four disagree raw purely over `t.co` expansion and media-link stripping. Spans are offsets into the canonical text, so expanded targets belong in entities, not substituted into the span. A fourth Tier 0 truncation found with it: 273 of 418 characters of an ordinary Persian post |
