...
**Last updated:** 2026-08-31 · Phase 0 in progress; `T-001`, `T-002`, `T-003`, and `T-006` complete
...
`src/x2knwldg/ids.py` (`T-003`), and the labelled run fixtures in `tests/fixtures/runs/` (`T-006`).
...
```bash
.venv/bin/python -m pytest -q               # expect: 187 passed, 16 subtests (plus new tests)
...
| **R15** Absolute host paths baked into `library/status.json` and `library/videos.json` | 🟡 Watch | `library.py` now emits an additive `relative_path` beside each absolute `path`. The absolute value is kept for the CLI and for humans; **the index must read `relative_path` and never trust `path`** |
...
Phase 0 now has four of its nine tasks done: `T-001` (ADRs), `T-002` (`schemas/v1/`), `T-003`
([`src/x2knwldg/ids.py`](../src/x2knwldg/ids.py)), and `T-006`
([run fixtures](../tests/fixtures/runs/README.md)). Remaining: `T-004`, `T-005`, `T-007`, `T-008`,
`T-009`.

`T-004` should delete `_project_run` from `tests/test_index_schemas.py` — that projection is a shape
probe standing in for the adapter — and have the real adapter produce the records the same tests
then check, over the fixtures **and** the sample. Build every identifier through `ids.py`; do not
assemble one with an f-string. Read `relative_path`, never `path`, from the library files (R15).
